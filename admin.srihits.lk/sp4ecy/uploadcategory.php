...
$cvb = mysqli_num_rows($query);
    if($cvb>0){
      ...
      $iQ = "insert into uploadcategory(createdby,userid,category,datetime)values('$category','$userid','$category','$datetime')";
    $chk=mysqli_query($con,$iQ);
    if($chk){$rtn=array('success'=>'true','message'=>'Category added Successfully','data'=>$iQ);}
