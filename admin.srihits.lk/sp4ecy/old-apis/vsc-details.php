...
$hj=mysqli_query($con,"select * from vsc where videoid='$unqid'");
$row = mysqli_num_rows($hj);
if($row>0){
    $query="update vsc set title='$title',slug='$slug',type='$seriestype',shortdescription='$shortdescrip',description='$desc',castcrew='$cacr',episode='$episode',season='$season',directors='$director',writers='$writer',languages='$languages',imdbrating='$rating',releasedate='$releDat',
    countries='$country',genre='$genre',runtime='$runtime',kids='$kid',email='$subs',push='$notif',publish='$publish',insertdatetime='$datetime',approve='$approve',price='$price',trailerid='$trailerid' where videoid='$unqid'";
    $chk = mysqli_query($con,$query);
if($chk){$rtn=array('success'=>'true','message'=>'Details updated Successfull');}
else{$rtn=array('success'=>'fail','message'=>'Details Not updated');}
}
else{
$query = "insert into vsc (userid,videoid,title,slug,type,shortdescription,description,castcrew,episode,season,languages,imdbrating,releasedate,countries,genre,runtime,kids,email,push,publish,insertdatetime,approve,price,trailerid,directors,writers) values
('$userid','$unqid','$title','$slug','$seriestype','$shortdescrip','$desc','$cacr','$episode','$season','$languages','$rating','$releDat','$country','$genre','$runtime','$kid','$subs','$notif','$publish','$datetime','$approve','$price','$trailerid','$director','$writer')";
$chk = mysqli_query($con,$query);
