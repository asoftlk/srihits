...
if($type=='list'){
    $query = mysqli_query($con,"select thumbnail.*,vsc.title,vsc.price, vsc.genre from thumbnail LEFT JOIN vsc ON vsc.videoid=thumbnail.videoid where vsc.publish='1' and vsc.approve='1' and vsc.type='$cate' and vsc.kids='$kids' ");
    $thumbnail=array();
    $poster=array();
...
$rtn = array('thumbnail'=>$thumbnail,'poster'=>$poster);
    ...
    $tmpDir=explode('|',$direc);
        ...
        $cjn=trim($tempCrew[0]," ");
      $query = mysqli_query($con,"select thumbnail.*,vsc.title,vsc.price from thumbnail LEFT JOIN vsc ON vsc.videoid=thumbnail.videoid where vsc.publish='1'and vsc.approve='1' and vsc.videoid !='$vid' and vsc.kids='$kids' and vsc.type='$cate' and castcrew like '%$cjn%'");
    while($row=mysqli_fetch_assoc($query)){
        if($row['type']=='thumbnail'){
            array_push($thumbnail,$row);
        }
    ...
    $query1 = mysqli_query($con,"select * from vsc where videoid ='$vid'");
    $details = array();
    $posThm=array();
