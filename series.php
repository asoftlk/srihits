...
<div class="movie">
    ...
    <?php
        ...
        $response->data->details[0]->releasedate;
        ...
        <div class="position-absolute moviecontent">
            ...
            <div class="text-light position-absolute pb-5 bottom-0 d-flex movieplay">';
                ...
                if($response->details[0]->trailer->url!=''){
                ...
                if(isset($_SESSION['userid'])){
                    if(in_array($response->data->details[0]->videoid, $watchlist)){
                        echo '<i class="bi bi-check-lg text-primary fs-2 watchlist" value="'.$response->data->details[0]->videoid.' , 1" ></i>';
                    }
                    else{
                        echo '<i class="bi bi-plus fs-2 watchlist" value="'.$response->data->details[0]->videoid.' , 1" ></i>';
                    }
                }
                else{
                    echo '<i class="bi bi-plus fs-2 watchlist" value="'.$response->data->details[0]->videoid.' , 1" ></i>';   
                ...
                <div class="text-light text-center ms-5 movieshare position-relative">
                    ...
                    <div class="sharelink position-absolute p-1" style="background-color:#192133; border-radius:10px; right:0">
                        <a onclick="MyWindow=window.open(\'https://www.facebook.com/sharer/sharer.php?u=https://www.srihits.veramasait.com/movie?id='.$id.'\',\'MyWindow\',\'width=600,height=400\'); return false;"><i class="bi bi-facebook ps-3 pe-3 fs-4 rounded"></i></a>
                        <a onclick="MyWindow=window.open(\'https://twitter.com/intent/tweet?url=https://www.srihits.veramasait.com/movie?id='.$id.'\',\'MyWindow\',\'width=600,height=400\'); return false;"><i class="bi bi-twitter ps-3 pe-3 fs-4 rounded"></i></a>
                        <a href="movie?id='.$_GET["id"].'"><i class="bi bi-link-45deg ps-3 pe-3 fs-4 rounded"></i></a>
                    </div>
                </div> 
            </div>
        </div>';
...
echo '<br style="clear:both"><div  class="container-fluid ps-3">';
        if(count($response->data->relateddata->thumbnail)>0){
            echo '<h5 class="text-light  ms-3 mt-5">Episodes</h5>
            ...
            for($j=0; $j<count($response->data->relateddata->thumbnail); $j++){
                ...
                echo '  <a href="series?id='.$response->data->relateddata->thumbnail[$j]->videoid.'"><div class="thumbnail position-relative pt-4 pb-2">
                        ...
                        <div class="thumbnailcontent position-absolute p-1 " >
                            ...
                            if(isset($_SESSION['userid'])){
                                if(in_array($response->data->relateddata->thumbnail[$j]->videoid, $watchlist)){
                                    echo '<button class="watchlist text-uppercase" value="'.$response->data->relateddata->thumbnail[$j]->videoid.', 1" href="#">&#x2713; REMOVE FROM WISHLIST</button>';
                                }
                                else{
                                    echo '<button class="watchlist text-uppercase" value="'.$response->data->relateddata->thumbnail[$j]->videoid.', 1" href="#">+ ADD TO WATCHLIST</button>';
                                }
                            }
                            else{
                             echo '<button class="watchlist" value="'.$response->data->relateddata->thumbnail[$j]->videoid.', " href="#">+ ADD TO WATCHLIST</button>';
                            }
                     echo  '</div>
                    </div></a>';
            }
