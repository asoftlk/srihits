...
$.ajax({
    ...
    success:function(data)
    {
        ...
        if(userid != ''){
                ...
                var wishlisttype =[];
                if(data.count>0){
                for(var j=0; j<data.thumbnail.length; j++){
                    wishlistid.push(data.thumbnail[j].videoid);
                }
                var json = data;
                if(json.thumbnail.length>0 && (offset==1)){
                    $(".title").append('<h4 class="title text-light text-capitalize ms-3 mt-2">Watchlist</h4>');
                }
                    ...
                    $(".title").append('<h4 class="title text-light text-center mt-4 text-capitalize ms-3 mt-2">No Data Found!</h4>');
                        ...
                        for(var i=0; i<json.thumbnail.length; i++){
                            if(data.thumbnail[i].nav=='vsc'){
                                nav = 'originals';
                            }
                            else{
                                nav = data.thumbnail[i].nav;
                            }
                    htmldata = htmldata + '\
                    <a href="'+nav+'?id='+json.poster[i].videoid+'"><div class="thumbnail position-relative">\
