...
$.ajax({
    ...
    success:function(data)
    {
        ...
        $.ajax({
            ...
            success:function(datanew){
                ...
                if(json.thumbnail.length>0 && (offset==1)){
                    $(".title").append('<h4 class="title text-light text-capitalize ms-3 mt-2"><?= $type ?></h4>');
                }
                ...
                for(var i=0; i<json.thumbnail.length; i++){
                    htmldata = htmldata + '\
                    <a href="originals?id='+json.poster[i].videoid+'"><div class="thumbnail position-relative">\
                    ...
                    if(wishlistid.includes(json.poster[i].videoid)){
                        htmldata = htmldata + '<button class="watchlist text-uppercase" value="'+json.poster[i].videoid+', 2"href="#">\u2713 REMOVE FROM WATCHLIST</button>';
                    }
                    else{
                        htmldata = htmldata + '<button class="watchlist text-uppercase" value="'+json.poster[i].videoid+', 2"href="#">+ ADD TO WATCHLIST</button>';
                    }
        ...
        for(var i=0; i<json.thumbnail.length; i++){
            ...
            <p class="thumbgenre text-light mb-0 text-capitalize">'+json.thumbnail[i].genre+'</p>\
            <button class="play" href="#"><i class="bi bi-play-fill"></i>&nbsp;Play</button>\
            <button class="watchlist text-uppercase" value="'+json.poster[i].videoid+', 2"href="#">+ ADD TO WATCHLIST</button>\
            </div>\
            </div></a>';
        }
        $(".list").append(htmldata);
